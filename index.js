...
let numContributors = GAMES_JSON.reduce( (total, game) => { 
...
let numContributions = GAMES_JSON.reduce( (total, game) => { 
    return game.pledged + total;
}, 0);

// set inner HTML using template literal
raisedCard.innerHTML = `$${numContributions.toLocaleString('en-US')}`;
...
const totalUnfundedGames = GAMES_JSON.reduce((total, game) => {
    game.goal > game.pledged ? ++count : count;
    return count;
}, 0);
...
let firstGameName = document.createTextNode(firstgame.name);
firstGameContainer.appendChild(firstGameName);

// do the same for the runner up item
let secondGameName = document.createTextNode(secondgame.name);
